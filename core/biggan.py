...
class BigGANGenerator(nn.Module):
    def __init__(self, out_sz:int, out_n_channels:int, up_blocks_n_ftrs:List[Tuple[int, int]],
                 ...
                 up_op:UpsamplingOperation2d=None):
        super().__init__()
        ...
        self.self_att = PooledSelfAttention2d(last_res_block_in_ftrs, init=_default_init)
        ...
        self.final_layers = nn.Sequential(
            ...
            conv_layer(last_res_block_out_ftrs, out_n_channels, bias=False, 
                       norm_type=NormType.Spectral, use_activ=False, init=_default_init),
            nn.Tanh()
        )
...
class BigResBlockUp(nn.Module):
    def __init__(self, in_ftrs:int, out_ftrs:int, cond_size:int, up_op:UpsamplingOperation2d=None,
        ...
        conv = functools.partial(conv_layer, bias=False, norm_type=NormType.Spectral, 
                                 use_activ=False, init=_default_init)

        self.bn1 = ConditionalBatchNorm2d(in_ftrs, cond_size, _default_init, _default_init)
        self.activ1 = activ
        self.upsample = up_op.get_layer(in_ftrs, in_ftrs)
        self.conv1 = conv(in_ftrs, out_ftrs)
        self.bn2 = ConditionalBatchNorm2d(out_ftrs, cond_size, _default_init, _default_init)
        self.activ2 = activ
...
class BigGANDiscriminator(nn.Module):
    def __init__(self, in_sz:int, res_blocks_n_ftrs:List[Tuple[int, int]], idx_block_self_att:int, 
        ...
        if activ is None: activ = nn.ReLU()
        # Params only passed when present, in order to preserve the defaults of BigResBlockDown
        down_blocks_kwargs = {}
        if down_op is not None: down_blocks_kwargs['down_op'] = down_op

        layers.extend([
            BigResBlockDown(in_ch, out_ch, activ=activ, apply_1st_activ=(i > 0),
                            **down_blocks_kwargs)
        ...
        layers.append(PooledSelfAttention2d(self_att_n_ch, init=_default_init))
        layers.extend([
            BigResBlockDown(in_ch, out_ch, activ=activ, **down_blocks_kwargs)
            for in_ch, out_ch in res_blocks_n_ftrs[idx_block_self_att:-1]
        ])
        final_n_ftrs = res_blocks_n_ftrs[-1][1]
        layers.append(BigResBlockDown(*res_blocks_n_ftrs[-1], False, activ=activ))
        layers.append(activ)
...
class BigResBlockDown(nn.Module):
    def __init__(self, in_ftrs:int, out_ftrs:int, downsample=True, down_op:DownsamplingOperation2d=None,
                 activ:nn.Module=None, apply_1st_activ=True):
        super().__init__()
        if activ is None: activ = nn.ReLU()
